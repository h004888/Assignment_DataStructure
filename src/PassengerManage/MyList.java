...
public class MyList {
    ...
    void addManyLast(Passenger[] a) {
        for (int i = 0; i < a.length; i++) {
            addLast(a[i]);
        }
    }

    void addFirst(Passenger x) {
        Node p = new Node(x);
        if (isEmpty()) {
            head = p;
            tail = p;
    ...
    Node searchByPcode(String code) {
        Node current = head;
        while (current != null) {
            if (current.info.getPcode().equalsIgnoreCase(code)) {
                return current;
            }
    ...
    Node searchByName(String code) {
        Node current = head;
        while (current != null) {
            if (current.info.getName().equalsIgnoreCase(code)) {
                return current;
            }
            current = current.next;
        }
    ...
    public Passenger searchPassengerByName(String name) {
        Node current = head;  // passengerHead là đầu của danh sách liên kết chứa các hành khách
        while (current != null) {
            if (current.info.name.equalsIgnoreCase(name)) {  // So sánh không phân biệt hoa thường
                return current.info;
            }
            current = current.next;
        }
